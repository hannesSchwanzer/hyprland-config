start_log_output() {
  ...
  (
    ...
    while true; do
      ...
      mapfile -t current_lines < <(tail -n $log_lines "$INSTALL_LOG_FILE" 2>/dev/null)
...
start_install_log() {
  ...
  sudo chmod 666 "$INSTALL_LOG_FILE"

  export OMARCHY_START_TIME=$(date '+%Y-%m-%d %H:%M:%S')

  echo "=== Omarchy Installation Started: $OMARCHY_START_TIME ===" >>"$INSTALL_LOG_FILE"
  start_log_output
}
...
stop_install_log() {
  ...
  if [[ -n ${INSTALL_LOG_FILE:-} ]]; then
    ...
    echo "=== Omarchy Installation Completed: $OMARCHY_END_TIME ===" >>"$INSTALL_LOG_FILE"
    echo "" >>"$INSTALL_LOG_FILE"
    echo "=== Installation Time Summary ===" >>"$INSTALL_LOG_FILE"
    ...
    if [ -f "/var/log/archinstall/install.log" ]; then
      ...
      if [ -n "$ARCHINSTALL_START" ] && [ -n "$ARCHINSTALL_END" ]; then
        ...
        echo "Archinstall: ${ARCH_MINS}m ${ARCH_SECS}s" >>"$INSTALL_LOG_FILE"
      fi
    fi
    ...
    if [ -n "$OMARCHY_START_TIME" ]; then
      ...
      echo "Omarchy:     ${OMARCHY_MINS}m ${OMARCHY_SECS}s" >>"$INSTALL_LOG_FILE"

      if [ -n "$ARCH_DURATION" ]; then
        TOTAL_DURATION=$((ARCH_DURATION + OMARCHY_DURATION))
        TOTAL_MINS=$((TOTAL_DURATION / 60))
        TOTAL_SECS=$((TOTAL_DURATION % 60))
        echo "Total:       ${TOTAL_MINS}m ${TOTAL_SECS}s" >>"$INSTALL_LOG_FILE"
      fi
    fi
    echo "=================================" >>"$INSTALL_LOG_FILE"

    echo "Rebooting system..." >>"$INSTALL_LOG_FILE"
  fi
}
...
run_logged() {
  ...
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] Starting: $script" >>"$INSTALL_LOG_FILE"

  # Use bash -c to create a clean subshell
  bash -c "source '$script'" </dev/null >>"$INSTALL_LOG_FILE" 2>&1

  local exit_code=$?

  if [ $exit_code -eq 0 ]; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Completed: $script" >>"$INSTALL_LOG_FILE"
    unset CURRENT_SCRIPT
  else
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Failed: $script (exit code: $exit_code)" >>"$INSTALL_LOG_FILE"
  fi
